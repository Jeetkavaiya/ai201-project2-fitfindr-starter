...
- Stores in `session["search_results"]`. Sets `session["selected_item"] = results[0]` (the Y2K Baby Tee).

**Step 3 — Outfit suggestion:**
Calls `suggest_outfit(session["selected_item"], wardrobe)` with the example wardrobe (10 items).
- Wardrobe is non-empty → uses the named-pieces prompt.
- LLM response: *"Pair this Y2K Baby Tee with your Baggy straight-leg jeans and Chunky white sneakers for a casual streetwear look with early-2000s energy. Alternatively, try it with your Wide-leg khaki trousers and Black combat boots for a grungier, more editorial vibe."*
- Stores in `session["outfit_suggestion"]`.

**Step 4 — Fit card:**
Calls `create_fit_card(session["outfit_suggestion"], session["selected_item"])`.
- LLM prompt includes the item title ("Y2K Baby Tee"), price ($18), platform (depop), and the outfit suggestion.
- Returns: *"I just scored this Y2K Baby Tee for $18 on depop and it goes with literally everything 🦋 Wide-leg jeans + combat boots = the only fit that matters rn."*
- Stores in `session["fit_card"]`.

**Final output to user:**
Three Gradio panels populate:
- **Panel 1 — Top listing:** "Y2K Baby Tee — Butterfly Print / $18.00 on Depop / Size: S/M | Condition: Good / Colors: white, pastel pink / Brand: Unknown brand / ..."
- **Panel 2 — Outfit idea:** The LLM's named-piece outfit suggestion from Step 3.
- **Panel 3 — Fit card:** The casual caption from Step 4.

**Error path (if Step 2 returned []):**
`session["error"]` is set to "No listings found matching max price $30. Try broadening your search..." — Panel 1 shows this message, Panels 2 and 3 are empty. `suggest_outfit` is never called.